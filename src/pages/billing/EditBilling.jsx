...
const EditBilling = () => {
 const { id } = useParams();
    ...
    const fetchBillingEdit = async () => {
      try {
        setIsButtonDisabled(true);
        const token = localStorage.getItem("token");
        const response = await axios.get(`${BASE_URL}/api/panel-fetch-billing-by-id/${id}`, {
          headers: {
    ...
    const handleSubmit = async (e) => {
      ...
      axios({
        url: BASE_URL + `/api/panel-update-billing/${id}`,
        method: "PUT",
        data,
        headers: {
        ...
        toast.success("Billing Updated Sucessfully");
