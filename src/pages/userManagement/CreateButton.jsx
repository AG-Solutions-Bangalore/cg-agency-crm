...
const CreateButton = () => {
  ...
  const buttonComponentsByPage = Object.entries(ButtonComponents).reduce((acc, [key, value]) => {
    if (!acc[value.page]) {
      acc[value.page] = [];
    }
  ...
  const availablePages = Object.keys(buttonComponentsByPage).filter(page => {
    ...
    return buttonsInComponent.some(button => !buttonsInUserControl.includes(button));
  });
  ...
  const validatePermissions = () => {
    ...
    getAvailableButtons(selectedPage).forEach(button => {
      ...
      const hasSelectedPermission = Object.values(buttonPermissions).some(value => value);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const permissionsToSubmit = Object.entries(permissions).map(([button, userPerms]) => {
      ...
      for (const permission of permissionsToSubmit) {
        await axios.post(
          "https://agsrebuild.store/public/api/panel-create-usercontrol",
          permission,
          {
            headers: {
      ...
      toast.error(error.response?.data?.message || "Failed to create user controls");
    } finally {
      setIsLoading(false);
    }
  };
                  ...
                  <tr key={button.value} className={validationErrors[button.value] ? "bg-red-50" : ""}>
                    ...
                    {userTypes.map((user) => (
                      <td key={user.id} className="px-4 py-3 text-sm">
                        <Checkbox
                          checked={permissions[button.value]?.[user.id] || false}
                          onChange={() => handlePermissionChange(button.value, user.id)}
                          color="blue"
                        />
                      </td>
                    ))}
