...
const OutstandingSupplierReport = () => {
    ...
    const fetchPayments = async () => {
      ...
      try {
        const response = await axios.post(
          `${BASE_URL}/api/panel-fetch-supplier-outstanding-report`,
          {
            from_date,
            to_date,
            billing_to_id,
        ...
        if (response.data.supplierOutstanding) {
          setPayments(response.data.supplierOutstanding);
          groupPaymentsByVendorAndBuyer(response.data.supplierOutstanding);
        }
      } catch (error) {
  ...
  const groupPaymentsByVendorAndBuyer = (payments) => {
    const grouped = payments.reduce((acc, payment) => {
      const { vendor_company, buyer_company } = payment;
      if (!acc[vendor_company]) {
        acc[vendor_company] = {};
      }
      if (!acc[vendor_company][buyer_company]) {
        acc[vendor_company][buyer_company] = [];
      }
      acc[vendor_company][buyer_company].push(payment);
      return acc;
    }, {});
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: "Supplier Outstanding Report",
    pageStyle: `
      @page {
        margin: 2mm;
      }
  ...
  const handlePdfDownload = () => {
    ...
    const docDefinition = {
      content: [
        {
          text: "Supplier Outstanding Report",
          style: "header",
          alignment: "center",
        },
        ...content,
    ...
    pdfMake
      ...
      .download("Supplier_Outstanding_Report.pdf");
  };
  ...
  const downloadReport = async () => {
    try {
      ...
      const res = await axios.post(
        `${BASE_URL}/api/panel-download-supplier-outstanding-report`,
        {
          from_date,
          to_date,
          billing_to_id,
      ...
      link.setAttribute("download", "Supplier_Outstanding_Report.csv");
      document.body.appendChild(link);
      link.click();

      toast.success("Supplier Outstanding Report downloaded successfully");
    } catch (err) {
    ...
    <Layout>
      <div className="p-6 max-w-screen bg-white">
        <div ref={reportRef} className="bg-white">
          ...
          {Object.entries(groupedPayments).map(([vendor, buyers]) => (
            <div key={vendor} className="mb-4">
              <h2 className="text-lg font-bold mb-2">Vendor: {vendor}</h2>

              {Object.entries(buyers).map(([buyer, payments]) => (
                <div key={buyer} className="mb-4">
                  <h3 className="text-md font-semibold mb-2">Buyer: {buyer}</h3>
                  <Table
                    striped
                    highlightOnHover
...
export default OutstandingSupplierReport;
